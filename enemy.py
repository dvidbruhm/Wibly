...
class Enemy(Creature):
    def __init__(self, position, rotation=90, scale=1, size=50, speed=100, turn_speed=5):
        ...
        self.head = Body(self, self.position, self.rotation, self.size*2, self.size*2, head=True, speed=speed)

        self.bodies.append(self.head)

        for i in range(4):
            b = Body(self, self.position, self.rotation, self.size, self.size, speed=30)
